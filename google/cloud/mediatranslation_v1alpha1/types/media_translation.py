...
class TranslateSpeechConfig(proto.Message):
    ...
    Attributes:
        ...
        alternative_source_language_codes (Sequence[str]):
            ...
            language detected in the audio. Note:

            1. If the provided alternative_source_language_code is not
               supported by current API version, we will skip that
               language code.
            2. If user only provided one eligible
               alternative_source_language_codes, the translation will
               happen between source_language_code and
               alternative_source_language_codes. The
               target_language_code will be ignored. It will be useful
...
class StreamingTranslateSpeechConfig(proto.Message):
    ...
    Attributes:
        ...
        stability (str):
            ...
            that stability and speed would be trade off.

            1. "LOW": In low mode, translation service will start to do
               translation right after getting recognition response. The
               speed will be faster.
            2. "MEDIUM": In medium mode, translation service will check
               if the recognition response is stable enough or not, and
               only translate recognition response which is not likely
               to be changed later.
            3. "HIGH": In high mode, translation service will wait for
               more stable recognition responses, and then start to do
               translation. Also, the following recognition responses
               cannot modify previous recognition responses. Thus it may
               impact quality in some situation. "HIGH" stability will
               generate "final" responses more frequently.
        translation_mode (str):
            Optional. Translation mode, the value should be "text",
            "audio", "text_and_audio". Default empty string will be
            treated as "text".

            1. "text": The response will be text translation. Text
               translation has a field "is_final". Detailed definition
               can be found in ``TextTranslationResult``.
            2. "audio": The response will be audio translation. Audio
               translation does not have "is_final" field, which means
               each audio translation response is stable and will not be
               changed by later response. Translation mode "audio" can
               only be used with "high" stability mode,
            3. "text_and_audio": The response will have a text
               translation, when "is_final" is true, we will also output
               its corresponding audio translation. When "is_final" is
               false, audio_translation field will be empty.
...
class StreamingTranslateSpeechResponse(proto.Message):
    ...
    Attributes:
        ...
        result (~.media_translation.StreamingTranslateSpeechResult):
            ...
            processed (For text translation, ``is_final`` could be
            ``true`` or ``false``. For audio translation, we do not have
            is_final field, which means each audio response is stable
            and will not get changed later. For text_and_audio, we still
            have ``is_final`` field in text translation, but we only
            output corresponsding audio when ``is_final`` is true.).
